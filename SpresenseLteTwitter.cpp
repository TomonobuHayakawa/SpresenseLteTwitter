/*
 SpresenseLteTwitter.cpp - Porting for Spresense LTE board.
 */
/*
...
SpresenseLteTwitter::SpresenseLteTwitter(const char *token) : lte(), client(), token(token)
{
    httpBody.reserve(100);
}

void SpresenseLteTwitter::begin(const char* apn, const char* user, const char* pass)
{
    while (true) {
...
bool SpresenseLteTwitter::post(const char *msg)
{
    statusCode = 0;
    parseStatus = 0;
    httpBody = "";
...
bool SpresenseLteTwitter::lastMention(void)
{
    httpBody.reserve(200);
    return post(NULL);
}

bool SpresenseLteTwitter::checkStatus(Print *debug)
{
    bool ret = true;
...
int SpresenseLteTwitter::wait(Print *debug)
{
